                            ...
                            <table class="table table-striped table-bordered dt-responsive nowrap" id="mydataTable" width="100%" cellspacing="0">
                                ...
                                @isset($courses)
                                    @foreach($courses as $key => $courses)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
<td class="text-truncate" style="max-width: 200px"><span title="{{ $courses->title }}">{{ $courses->title }}</span></td>
                                            <td class="text-truncate" style="max-width: 200px"><span title="{{ $courses->description }}">{{ $courses->description }}</span></td>
                                            
                                            <td class="text-center">
                                                <div class="d-inline-flex">
                                                    <a href="{{ route('Module-TaskManagerCourseGetFind', $courses->uuid) }}" title="Consulter"
                                                        data-id= "{{ $courses->id }}"
                                                        data-uuid= "{{ $courses->uuid }}"
                                                    ...
                                                    <a href="{{ route('Module-TaskManagerCoursePostEdit', $courses->uuid) }}" title="Editer"
                                                        data-id= "{{ $courses->id }}"
                                                        data-uuid= "{{ $courses->uuid }}"
                                                        data-title= "{{ $courses->title }}"
                                                        data-description= "{{ $courses->description }}"
                                                    ...
                                                    {{-- <button class="btn btn-sm btn-outline-success modalEdit mr-1 btnEditCourse" title="Editer">
                                                        <i class="fa fa-edit"></i>
                                                    </button> --}}
                                                    <button type="submit" class="btn btn-sm btn-outline-danger delete" data-uuid="{{ $courses->uuid }}" title="Supprimer">
