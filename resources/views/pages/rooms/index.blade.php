                            ...
                            <table class="table table-striped table-bordered dt-responsive nowrap" id="mydataTable" width="100%" cellspacing="0">
                                ...
                                @isset($rooms)
                                    @foreach($rooms as $key => $rooms)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
<td class="text-truncate" style="max-width: 200px"><span title="{{ $rooms->title }}">{{ $rooms->title }}</span></td>
                                            <td class="text-truncate" style="max-width: 200px"><span title="{{ $rooms->description }}">{{ $rooms->description }}</span></td>
                                            
                                            <td class="text-center">
                                                <div class="d-inline-flex">
                                                    <a href="{{ route('Module-TaskManagerRoomGetFind', $rooms->uuid) }}" title="Consulter"
                                                        data-id= "{{ $rooms->id }}"
                                                        data-uuid= "{{ $rooms->uuid }}"
                                                        data-title= "{{ $rooms->title }}"
                                                    ...
                                                    <a href="{{ route('Module-TaskManagerRoomPostEdit', $rooms->uuid) }}" title="Editer"
                                                        data-id= "{{ $rooms->id }}"
                                                        data-uuid= "{{ $rooms->uuid }}"
                                                        data-title= "{{ $rooms->title }}"
                                                        data-description= "{{ $rooms->description }}"
                                                    ...
                                                    {{-- <button class="btn btn-sm btn-outline-success modalEdit mr-1 btnEditRoom" title="Editer">
                                                        <i class="fa fa-edit"></i>
                                                    </button> --}}
                                                    <button type="submit" class="btn btn-sm btn-outline-danger delete" data-uuid="{{ $rooms->uuid }}" title="Supprimer">
